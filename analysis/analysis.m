...
path='';strnum='33';str1='data_';str2='_shortburst';str3='';
data=load(fullfile(path,strcat(str1,strnum,str2,str3)));
...
SOM_cell_freq_mean=mean(cell_freq(num+inter_num+1:end));
SOM_cell_freq_std=std(cell_freq(num+inter_num+1:end));
SOM_cell_freq=cell_freq(num+inter_num+1:end);
...
figure (12121)
A_curve=[0:0.05:100];
[nb,xb]=hist(plastic_cell_freq,A_curve);
curveplot=[];curveplot=nb/numel(plastic_cell_freq)
% plot(xb_A(:,I),yyy)
smoothcurve=smooth(curveplot);
hold on
plot(xb,smoothcurve,'Color',[0.4 0.4 0.8],'Linewidth',2);
% nb_A_plot=xb_A(:,I)';
% xb_A_plot=nb_A(:,I)';
...
figure (12121)
A_curve=[0:0.3:100];
[nb,xb]=hist(ITN_cell_freq,A_curve);

curveplot=[];curveplot=nb/numel(ITN_cell_freq);
% plot(xb_A(:,I),yyy)
smoothcurve=smooth(curveplot);
hold on
plot(xb,smoothcurve,'Color',[0.8 0.4 0.4],'Linewidth',2);

figure (1)
[nb,xb]=hist(SOM_cell_freq,A);
[X,I]=find(nb>0);
hold on; bh=bar(xb(:,I),nb(:,I)/numel(SOM_cell_freq),'EdgeColor','none','BarWidth',1);
set(bh,'facecolor',[0.6 0.8 0.6]);

    
set(gca,'xscale','log');xlabel('Hz'); 
xlim([10^(-2) 10^(2)]);
x=sprintf('Spiking freq histogram for CPs is %3.2fHz+/-%3.2fHz;PVs is %3.2fHz+/-%3.2fHz; SOMs is %3.2fHz+/-%3.2fHz; ',plastic_cell_freq_mean,plastic_cell_freq_std,ITN_cell_freq_mean,ITN_cell_freq_std,SOM_cell_freq_mean,SOM_cell_freq_std);
...
figure (12121)
A_curve=[0:0.2:100];
[nb,xb]=hist(SOM_cell_freq,A_curve);
curveplot=[];curveplot=nb/numel(SOM_cell_freq)
% plot(xb_A(:,I),yyy)
smoothcurve=smooth(curveplot);
hold on
plot(xb,smoothcurve,'Color',[0.6 0.8 0.6],'Linewidth',2);

set(gca,'xscale','log');xlabel('Hz'); 
xlim([10^(-2) 10^(2)]);
x=sprintf('Spiking freq histogram curve for CPs is %3.2fHz+/-%3.2fHz;PVs is %3.2fHz+/-%3.2fHz; SOMs is %3.2fHz+/-%3.2fHz; ',plastic_cell_freq_mean,plastic_cell_freq_std,ITN_cell_freq_mean,ITN_cell_freq_std,SOM_cell_freq_mean,SOM_cell_freq_std);
title(x);ylabel('Percentage(#)');
legend('PN','ITN','SOM')


spikefreqinfo=cell(2,3);
spikefreqinfo(1,1:3)={'CPs','PVs','SOMs'};
%%%%fill in CPs
...
%%%%fill in SOMs
cell_temp=[];cell_temp=cell(som_num+1,2);
cell_temp(1,1:2)={'Freq(Hz)','IDs'};
cell_temp(2:end,:)=num2cell([cell_freq(num+inter_num+1:end),[num+inter_num+1:all_num]']);
spikefreqinfo{2,3}=cell_temp;
