...
#define nrn_init _nrn_init__IZH
#define _nrn_initial _nrn_initial__IZH
#define nrn_cur _nrn_cur__IZH
#define _nrn_current _nrn_current__IZH
#define nrn_jacob _nrn_jacob__IZH
#define nrn_state _nrn_state__IZH
#define _net_receive _net_receive__IZH 
#define states states__IZH 
...
#define vrACH _p[11]
#define Cap _p[12]
#define alphaShutdown _p[13]
#define aMin _p[14]
#define aMax _p[15]
#define bACH _p[16]
#define aACH _p[17]
#define g _p[18]
#define ACHshutdown _p[19]
#define u _p[20]
#define Du _p[21]
#define v _p[22]
#define _g _p[23]
...
#define ACH ACH_IZH
 double ACH = 1;
#define a_OLM_IZH _thread1data[0]
#define a_OLM _thread[_gth]._pval[0]
#define uinit uinit_IZH
 double uinit = 0;
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 pnt_receive_size[_mechtype] = 1;
 	...
 	ivoc_help("help ?1 IZH /home/pbcanfield/Desktop/NeuroResearch/CA3Cell_Izh/x86_64/izh.mod\n");
...
/*CVODE*/
 static int _ode_spec1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {int _reset = 0; {
   Du = ( a_OLM * ACHshutdown + a * alphaShutdown ) * ( b * ( v - ( vr + vrACH * ( - 1.0 + ACH ) ) ) - u ) ;
   }
 return _reset;
}
...
{  double* _p; Datum* _ppvar; Datum* _thread; _NrnThread* _nt;
   ...
   if (_lflag == 1. ) {*(_tqitem) = 0;}
 {
   if ( _lflag  == 1.0 ) {
    ...
    int __i, __neq = 1;
    double __state = u;
    double __primary_delta = (u + d + dACH * ( 1.0 - ACH )) - __state;
    double __dtsav = dt;
...
static double _nrn_current(double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt, double _v){double _current=0.;v=_v;{ {
   vv = - ( k * ( v - ( vr + vrACH * ( - 1.0 + ACH ) ) ) * ( v - vt ) - u ) ;
   a_OLM = 0.023 * pow( ACH , 2.0 ) - 0.022 * ACH + 0.002 ;
   }
 _current += vv;
...
#if NMODL_TEXT
static const char* nmodl_filename = "/home/pbcanfield/Desktop/NeuroResearch/CA3Cell_Izh/modfiles/izh.mod";
static const char* nmodl_file_text = 
  "NEURON {\n"
  "  POINT_PROCESS IZH\n"
