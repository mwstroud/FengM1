...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 4, "cvodeieq");
 	...
 	ivoc_help("help ?1 capool /home/mizzou/Documents/NeuroResearch/bmtk_tutorial/components/mechanisms/modfiles/x86_64/capool.mod\n");
...
/*CVODE*/
 static int _ode_spec1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {int _reset = 0; {
   Dcai = - fca * ica / ( z * FARADAY * w * 1e-4 ) + ( cainf - cai ) / tauca ;
   }
 return _reset;
}
 static int _ode_matsol1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {
 Dcai = Dcai  / (1. - dt*( ( ( ( - 1.0 ) ) ) / tauca )) ;
  return 0;
}
 /*END CVODE*/
 static int states (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) { {
    cai = cai + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / tauca)))*(- ( ( ( - fca )*( ica ) ) / ( z * FARADAY * w * 1e-4 ) + ( ( cainf ) ) / tauca ) / ( ( ( ( - 1.0 ) ) ) / tauca ) - cai) ;
   }
  return 0;
}
...
#if NMODL_TEXT
static const char* nmodl_filename = "/home/mizzou/Documents/NeuroResearch/bmtk_tutorial/components/mechanisms/modfiles/capool.mod";
static const char* nmodl_file_text = 
  ": Ca2+ pool for IKCa\n"
  "\n"
  "NEURON {\n"
